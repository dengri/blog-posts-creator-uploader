...
$img_folder = new Sitecontent\Folder\Folder(SERVER_IMG_LARGE_PATH );
...
//	$client->debug = true;
	$image_uploaded = $client->upload_image(SERVER_IMG_LARGE_PATH . $image);

	//Update db with cover ID and URL
	$db->saveImageInfoToDB(array('img_path' => $image, 
															 'img_url'  => $image_uploaded['url']), 'large');
